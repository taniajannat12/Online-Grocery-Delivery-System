...
"use client"; // ক্লায়েন্ট কম্পোনেন্টের জন্য এটি বাধ্যতামূলক
import { useState } from 'react';
import { useRouter } from 'next/navigation'; // সঠিক নেভিগেশন হুক
...
const SignPage = () => {
  ...
  const handleSignup = (e) => {
    ...
    // localStorage শুধু ব্রাউজারে থাকে, তাই "use client" এর ভেতরে এটি কাজ করবে
    const existingUsers = JSON.parse(localStorage.getItem("users") || "[]");
    existingUsers.push(data);
    localStorage.setItem("users", JSON.stringify(existingUsers));
    alert("Account Created! Ready to shop.");
    router.push('/login'); // পেজ পরিবর্তনের জন্য router.push
  };
